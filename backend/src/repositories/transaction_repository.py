...
class TransactionRepository:
    ...
    def get_monthly_history(self, start_date: date) -> List[Tuple[int, int, Decimal, Decimal]]:
    ...
    def get_expenses_by_category(self, start_date: date, end_date: date) -> List[Tuple[str, str, str, Decimal]]:
        return self.db.query(
            Category.name,
            Category.color,
            Category.icon,
    ...
    def get_recent_transactions(self, limit: int = 10) -> List[Transaction]:
        return self.db.query(Transaction).options(joinedload(Transaction.category)).order_by(
