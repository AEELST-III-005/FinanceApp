...
def test_get_dashboard_summary_refactored(dashboard_service, mock_transaction_repository):
    ...
    with patch('backend.src.services.dashboard_service.date') as mock_date:
        ...
        response = dashboard_service.get_dashboard_summary(DashboardPeriod.CURRENT_MONTH)
