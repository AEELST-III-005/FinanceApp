...
def test_get_transactions_with_filters(client):
    # Arrange
    cat_response = client.post("/api/categories/", json={"name": "Food", "icon": "f", "color": "c"})
    ...
    response = client.get(f"/api/transactions/?transaction_type=income")
    assert len(response.json()) == 1
    assert response.json()[0]["title"] == "New Income"

    # Act & Assert - Filter by date
    response = client.get(f"/api/transactions/?start_date=2024-04-01")
    assert len(response.json()) == 1
    assert response.json()[0]["title"] == "New Income"
...
def test_update_transaction_not_found(client):
    ...
    # Act
    response = client.put("/api/transactions/non-existent", json={
        "title": "Lunch",
        "amount": "15.50",
        "transaction_date": "2024-04-22",
        "transaction_type": "expense",
